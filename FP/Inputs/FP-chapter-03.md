...
// 箭头函数版本
const partial = (fn, ...presetArgs) => (...nextArgs) =>
...
```js
const getCurrentUser = partial(ajax, "http://some.api/person", {
...
add 函数签名不是 map 函数所预期的，因此在其之上封装一层 adder 用以管理函数输入。但仔细观察，其中的模式跟上面的 partial 很象，因此我们可以使用 partial 来调整 add 函数签名，以符合 map 函数的预期。
...
partialRight 只能确保一些值被当做原函数最右边的实参传入，却不能保证特定的形参接收特定的被偏应用的值。
...
> JavaScript 中，偏应用与柯里化都使用闭包来保存实参，知道收齐到所有实参后我们再执行原函数。
